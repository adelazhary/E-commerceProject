<div class="w-full bg-white rounded-lg shadow-md overflow-hidden flex flex-col items-center justify-between p-4">
  <a href="{{ route('product.show', $product->id) }}">
    <img class="w-full h-48 object-cover rounded-t-lg" src="{{ $product->image }}" alt="{{ $product->title }}">
  </a>
  <div class="flex flex-col justify-between p-4">
    ...
    <a href="{{ route('cart.add', $product->id) }}" class="mt-4 inline-flex items-center px-4 py-2 text-white bg-blue-500 rounded-md hover:bg-blue-700">
      Add to Cart
